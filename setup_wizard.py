...
if sys.version_info < (3, 11):
    print(f"ERROR: Python 3.11 or newer is required. You have {sys.version}")
...
class SetupWizard(tk.Tk):
    ...
    def _create_windows_task(self):
        ...
        from email_manager.config_writer import windows_task_script

        time_str = self.data["digest_time"].get().strip() or "09:00"
        script_path = Path(tempfile.gettempdir()) / "email_manager_create_task.ps1"
        script_path.write_text(windows_task_script(Path(APP_DIR), time_str), encoding="utf-8")
        try:
            subprocess.Popen([
    ...
    def _create_mac_launchd(self):
        from pathlib import Path
        from email_manager.config_writer import launchd_plist
        ...
        plist_path = plist_dir / "com.emailmanager.daily.plist"
        (Path(APP_DIR) / "logs").mkdir(parents=True, exist_ok=True)
        plist_path.write_text(launchd_plist(Path(APP_DIR), PYTHON_BIN, hour, minute), encoding="utf-8")
        try:
    ...
    def _write_config(self):
        ...
        for acct in self.data["accounts"]:
            ...
            if not name or not email:
                continue
            accounts.append(WizardAccount(
                name=name,
                email=email,
                provider=acct["provider_var"].get(),
                imap_server=acct["imap_var"].get().strip() if "imap_var" in acct else None,
                smtp_server=acct["smtp_var"].get().strip() if "smtp_var" in acct else None,
                password=acct["password_var"].get().strip() if "password_var" in acct else None,
            ))

        entities = []
        for ent in self.data["entities"]:
            ename = ent["name"].get().strip()
            if not ename:
                continue
            raw_kw = ent["keywords"].get().strip()
            entities.append(WizardEntity(
                name=ename,
        ...
        data = WizardData(
            ...
            api_key=self.data["api_key"].get().strip(),
            ms_client_id=self.data["ms_client_id"].get().strip(),
            accounts=accounts,
            entities=entities,
            labels=list(self.data.get("labels", [])),
            digest_email=self.data["digest_email"].get().strip(),
            digest_time=self.data["digest_time"].get().strip(),
            digest_account=self.data["digest_account"].get().strip(),
        )
        from pathlib import Path
        write_config_files(Path(APP_DIR), data)
